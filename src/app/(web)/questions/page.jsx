...
export default function QuestionsListPage() {
    ...
    const [maxResult, setMaxResult] = useState(12); // Items per page
    const [tags, setTags] = useState([]);
    const [selectedTags, setSelectedTags] = useState([]);

    const fetchQuestions = (page, maxResult) => {
        setLoading(true);
        const tagIdsQuery = selectedTags.length > 0 ? `&tagIds=${selectedTags.map(tag => tag.id).join(",")}` : "";

        HttpClient.get(`/questions?page=${page}&maxResult=${maxResult}${tagIdsQuery}`)
            .then(res => res.json())
            .then((data) => {
        ...
        CacheProvider.fetchData("tags", 5, () => HttpClient.get("/tags"))
    ...
    const SkeletonLoader = () => (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {[...Array(12)].map((_, index) => (
                ...
                >
                    {/* İlk satır tam genişlik */}
                    <div className="h-6 bg-gray-300 rounded mb-4"></div>
                    {/* İkinci satır yarım genişlik */}
                    <div className="h-4 bg-gray-300 rounded mb-2 w-1/2"></div>
                    {/* En altta hizalanmış üçüncü satır */}
    ...
    const QuestionNotFound = () => (
        <div
            className="flex flex-col items-center justify-center min-h-[300px] bg-white shadow-md rounded-lg p-6">
            <img
                                ...
                                {filteredQuestions?.map((question) => (
                                    ...
                                    >
                                        <div>
                                            ...
                                            <div>
                                                {question.answer.length > 10
                                                    ? `${question.answer.slice(0, 10)}...`
                                                    : question.answer}
                                            </div>
                                            ...
                                            {question.tags?.length > 0 && (
                                                <p className="text-sm text-gray-500">
                                                    ...
                                                    {question.tags.map((c) => `#${c.name}`).join(", ")}
                                                </p>
                                            )}
