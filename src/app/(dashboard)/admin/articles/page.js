...
function ArticleList() {
  ...
  const deleteSelectedArticles = async () => {
    ...
    try {
      ...
      await Promise.all(selectedArticles.map((articleId) => HttpClient.delete(`/articles/${articleId}`)))
      toast.current.show({
        severity: "success",
        summary: "Success",
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        <Button onClick={() => route("/admin/articles/create")}>Create</Button>
        <div className={"flex"}>
          <InputNumber value={findById} onValueChange={(e) => setFindById(e.target.value)} />
          <Button onClick={goFindById}>Tap </Button>
        </div>
        <Button onClick={deleteSelectedArticles} className="bg-red-500 hover:bg-red-700 text-white">
          Delete Selected
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {articles.map((article) => (
          <div key={article.id} className="border rounded-lg shadow-md overflow-hidden relative">
            ...
            <div className="p-4">
              <h2 className="text-xl font-semibold mb-2 cursor-pointer" onClick={() => onClick(article.id)}>
                {article.title}
              </h2>
              <p className="text-gray-600 mb-2">Categories: {article.categories || "No Categories"}</p>
              <p className="text-gray-600 mb-4">Authors: {article.authorName || "No Authors"}</p>
      ...
      <div className="flex justify-center mt-4 space-x-2">
        <button className="px-3 py-1 border rounded" onClick={goToPreviousPage} disabled={page === 0}>
          &laquo; Previous
        </button>
        ...
        <button className="px-3 py-1 border rounded" onClick={goToNextPage} disabled={page === totalPages - 1}>
          Next &raquo;
        </button>
      </div>
    </div>
