...
const CategoryList = () => {
    return (
        <div className='question_categories  dark:bg-gray-900  flex w-max bg-white items-center mx-4  rounded-3xl  py-3 px-6'>
            <input type="text" className="bg-gray-50 border dark:bg-gray-700 dark:border-gray-600   outline-none  border-gray-400 text-gray-900 dark:text-gray-800 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 sticky top-0 block  p-2.5  dark:placeholder-gray-400   dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Axtar" />
            <div className='flex px-2 justify-center items-center overflow-auto categories_container'  >
                {
                    categories.filter(category => category.parentId === 0).map((parent) => (
                        <div key={parent.id} className="hs-dropdown  relative inline-flex">
                            <div id="hs-dropdown-with-icons" key={parent.id} className="flex  hs-dropdown-toggle items-center justify-between p-2.5 rounded-md hover:bg-blue-100 bg-blue-50 dark:bg-gray-500 hover:dark:bg-gray-600 mr-1 cursor-pointer">
                                <div className="flex items-center">
                                    <p className="text-sm text-gray-800  dark:text-gray-300     font-semibold ml-2">{parent.name}</p>
                                </div> 
                            </div>
                            <div 
